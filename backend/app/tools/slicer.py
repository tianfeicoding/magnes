...
async def slicer_node(state: MagnesState):
    ...
    # 调用 302.ai 视觉分析工具
    # 使用中央 Prompt 库中的指令
    analyzer_prompt = state.get("instruction") or f"请将这张图片拆分为 {num_layers} 个可编辑的透明图层，尽量分离主体、文字、装饰元素和背景。"
    ...
    if "error" in raw_analysis:
        error = raw_analysis.get("error") or "unknown slicer error"
        print(f"[Asset Slicer] ❌ 分层失败: {error}")
        return {"current_step": "error_slicer_failed", "error": f"分层接口失败: {error}"}
