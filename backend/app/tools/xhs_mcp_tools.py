...
class XHSCLITools:
    ...
    CLI_TIMEOUT = 120.0
    LOGIN_CLI_TIMEOUT = float(os.getenv("XHS_LOGIN_CLI_TIMEOUT", "15"))
    SEARCH_CLI_TIMEOUT = float(os.getenv("XHS_SEARCH_CLI_TIMEOUT", "45"))
    DETAIL_CLI_TIMEOUT = float(os.getenv("XHS_DETAIL_CLI_TIMEOUT", "25"))
    ...
    async def _run_cli(
        ...
        *args,
        ...
        try:
            ...
            effective_timeout = timeout or cls.CLI_TIMEOUT
            ...
            effective_timeout = timeout or cls.CLI_TIMEOUT
            print(f"[XHS CLI Tool] ❌ CLI 超时 ({effective_timeout}s): {subcommand}")
            await cls._force_stop_process(proc, subcommand)
            raise RuntimeError(f"CLI 执行超时 ({effective_timeout}s)")

    @classmethod
    async def _force_stop_process(cls, proc, subcommand: str) -> None:
        """硬停止 CLI 进程组，避免子进程/浏览器桥接调用卡住后阻塞主链路。"""
        if not proc:
            return

        async def _wait_short(label: str) -> bool:
            try:
                await asyncio.wait_for(proc.wait(), timeout=2)
                print(f"[XHS CLI Tool] ✅ CLI 进程已停止 ({label}): {subcommand}")
                return True
            except asyncio.TimeoutError:
                return False
            except Exception as e:
                print(f"[XHS CLI Tool] ⚠️ 等待 CLI 进程停止失败 ({label}): {e}")
                return True
        ...
        try:
            ...
            try:
                ...
                pass
        if await _wait_short("terminate"):
            return
        ...
        try:
            ...
            try:
                ...
                pass
        await _wait_short("kill")

    @classmethod
    async def check_login(cls) -> dict:
        """执行 check-login，并保留退出码用于上层判断。"""
        return await cls._run_cli(
            "check-login",
            allow_exit_codes={0, 1},
            timeout=cls.LOGIN_CLI_TIMEOUT,
        )
    ...
    async def search_feeds(cls, keyword: str):
        ...
        data = await cls._run_cli(
            ...
            "--keyword",
            keyword,
            timeout=cls.SEARCH_CLI_TIMEOUT,
        )
    ...
    async def get_feed_detail(cls, feed_id: str, xsec_token: str):
        ...
        data = await cls._run_cli(
            ...
            "--xsec-token",
            xsec_token,
            timeout=cls.DETAIL_CLI_TIMEOUT,
        )
        return data
...
class XHSMCPTools:
    ...
    async def get_feed_detail(feed_id: str, xsec_token: str, load_all_comments: bool = False, limit: int = 10):
        ...
        try:
            ...
            if "超时" in err_str or "timeout" in lowered:
                code = "detail_timeout"
                title = "详情抓取超时"
            elif "验证" in err_str or "扫码" in err_str:
                code = "verification_required"
                title = "小红书触发验证"
            elif "没有捕获到 feed 详情数据" in err_str or "no_detail_map" in lowered:
                code = "no_detail_map"
                title = "详情数据未写入页面"
            elif "不可访问" in err_str or "page isn't available" in lowered:
