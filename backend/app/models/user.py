...
class User(Base):
    ...
    memory_events = relationship("MemoryEvent", cascade="all, delete-orphan")
    memory_analysis_state = relationship("MemoryAnalysisState", cascade="all, delete-orphan", uselist=False)
    task_traces = relationship("TaskTrace", cascade="all, delete-orphan")
    skill_candidates = relationship("SkillCandidate", cascade="all, delete-orphan")
