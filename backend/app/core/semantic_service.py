...
SEMANTIC_EXTRACT_TIMEOUT = float(os.getenv("SEMANTIC_EXTRACT_TIMEOUT", "20"))
...
async def extract_semantic_content(text: str) -> List[Dict]:
    ...
    try:
        ...
        result = await asyncio.wait_for(
            ...
            timeout=SEMANTIC_EXTRACT_TIMEOUT,
        )
        return [item.model_dump() for item in result.items]
    except asyncio.TimeoutError:
        print(f"[Semantic Service] ⚠️ Extraction timeout ({SEMANTIC_EXTRACT_TIMEOUT}s), fallback to raw reply")
        return []
