...
from app.memory.models import CanvasActionLog, UserMemory, MemoryEvent
...
def _merge_memory_evidence(existing: Optional[str], incoming: Optional[str], limit: int = 5) -> str:
    items = []
    for raw in [existing, incoming]:
        ...
        for line in str(raw).splitlines():
            ...
            if line and line not in items:
                items.append(line)
    return "\n".join(items[-limit:])[-800:]


async def _analyze_memory_events_with_llm(events: List[MemoryEvent]) -> MemoryAnalysisResult:
    """调用 LLM 分析新增事件，提取用户偏好和稳定工作流。"""
    ...
    event_texts = []
    for event in events:
        ts = event.created_at.strftime("%m-%d %H:%M") if event.created_at else ""
        payload = event.payload or {}
        content = event.content or payload.get("description") or payload.get("action") or ""
        if not content:
            continue
        event_texts.append(f"[{ts}] {event.event_type}/{event.role or '-'}: {content}")
    event_content = "\n".join(event_texts)
...
"""
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        ("user", "以下是用户最近新增的对话、规划和画布事件，请分析并提取稳定偏好或工作流：\n\n{events}")
    ])

    chain = prompt | llm.with_structured_output(MemoryAnalysisResult)
    result = await chain.ainvoke({"events": event_content})
    return result


async def _analyze_logs_with_llm(logs: List[CanvasActionLog]) -> MemoryAnalysisResult:
    """兼容旧预览接口：调用 LLM 分析操作日志，提取用户偏好"""
    pseudo_events = []
    for log in logs:
        event = MemoryEvent(
            user_id=log.user_id,
            conversation_id=log.conversation_id,
            event_type="canvas_action",
            role=None,
            content=log.description,
            payload=log.payload or {},
            source="canvas_action_log",
            created_at=log.created_at,
        )
        pseudo_events.append(event)
    return await _analyze_memory_events_with_llm(pseudo_events)
...
async def analyze_user_memory(
    ...
    min_events = int(data.get("minEvents", 3))

    state = await memory_service.get_or_create_analysis_state(db, user.id)

    filters = [MemoryEvent.user_id == user.id]
    if state.last_event_created_at:
        filters.append(
            or_(
                MemoryEvent.created_at > state.last_event_created_at,
                and_(
                    MemoryEvent.created_at == state.last_event_created_at,
                    MemoryEvent.id != state.last_event_id,
                )
            )
        )
    ...
    if not events:
        return {"status": "success", "data": {"extracted": [], "message": "暂无新增事件可供分析", "eventCount": 0}}

    if len(events) < min_events:
        latest = events[-1]
        state.last_event_created_at = latest.created_at
        state.last_event_id = latest.id
        state.analyzed_event_count = (state.analyzed_event_count or 0) + len(events)
        await db.commit()
        return {"status": "success", "data": {"extracted": [], "message": "新增事件不足，已推进分析游标", "eventCount": len(events)}}

    # 2. 调用 LLM 分析
    try:
        analysis = await _analyze_memory_events_with_llm(events)
    except Exception as e:
    ...
    for pref in analysis.preferences:
        ...
        if existing:
            ...
            existing.evidence = _merge_memory_evidence(existing.evidence, pref.evidence)
            ...
            memory = UserMemory(
                ...
                source_conversation_id=events[0].conversation_id if events else None
            )
            db.add(memory)
