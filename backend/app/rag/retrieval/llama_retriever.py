...
class RankBM25Retriever(BaseRetriever):
    """
    LlamaIndex BM25 插件未安装时的本地降级实现。
    项目 requirements 已包含 rank-bm25，可避免检索阶段因可选插件缺失而直接失败。
    """
    def __init__(
        self,
        nodes: List,
        ...
        tokenized_corpus = [self._tokenizer(node.get_content()) for node in nodes]
        self._bm25 = BM25Okapi(tokenized_corpus)

    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        query_tokens = self._tokenizer(query_bundle.query_str)
        scores = self._bm25.get_scores(query_tokens)
        ranked = sorted(enumerate(scores), key=lambda item: item[1], reverse=True)

        results = []
        for idx, score in ranked[:self._similarity_top_k]:
            if score <= 0:
                continue
            results.append(NodeWithScore(node=self._nodes[idx], score=float(score)))
        return results


def _build_bm25_retriever(nodes: List, tokenizer: Callable[[str], List[str]], top_k: int):
    try:
        from llama_index.retrievers.bm25 import BM25Retriever

        print("DEBUG: [llama_retriever.py] 🧩 Using llama-index-retrievers-bm25")
        ...
        if exc.name != "llama_index.retrievers":
            raise
        print("DEBUG: [llama_retriever.py] ⚠️ llama-index BM25 plugin missing, using rank-bm25 fallback")
