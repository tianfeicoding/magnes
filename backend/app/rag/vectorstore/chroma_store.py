...
def _build_where_filter(**conditions) -> Optional[dict]:
    """Build a Chroma-compatible where filter.

    Chroma rejects plain multi-key filters such as
    {"doc_id": "...", "user_id": "..."}. Use $and for multi-key filters.
    """
    parts = [{key: value} for key, value in conditions.items() if value not in (None, "")]
    if not parts:
        return None
    if len(parts) == 1:
        return parts[0]
...
async def delete_knowledge_document(doc_id: str, user_id: str = None) -> int:
    ...
    collection = get_knowledge_collection()
    try:
        where_filter = _build_where_filter(doc_id=doc_id, user_id=user_id)
...
async def get_knowledge_documents(limit: int = 100, offset: int = 0, category: str = None, user_id: str = None) -> list[dict]:
    ...
    collection = get_knowledge_collection()
    try:
        where_filter = _build_where_filter(category=category, user_id=user_id)
