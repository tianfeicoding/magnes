...
async def chunk_document_with_llama(
    ...
    # 提取全局摘要和标签。upload 流程已经在 ParsedDocument.enrich_with_llm()
    # 里生成过一次，这里优先复用，避免再次走 RAG Config 默认 LLM 导致令牌不一致。
    global_summ = (parsed.summary or "").strip()
    global_tags = [str(t).strip() for t in (parsed.tags or []) if str(t).strip()]

    if global_summ or global_tags:
        print(f"[DocChunker] ✨ 复用解析阶段摘要/标签: tags={len(global_tags)}, summary={global_summ[:50]}...")
    else:
        print("[DocChunker] 🚀 解析阶段无摘要/标签，初始化 GlobalSummarizer 兜底生成...")
        ...
        global_summ = (global_info.get("summary") or "").strip()
        global_tags = [str(t).strip() for t in (global_info.get("tags") or []) if str(t).strip()]
        print(f"[DocChunker] ✨ 已生成全局摘要: {global_summ[:50]}...")

    if not global_summ:
        fallback_text = re.sub(r"\s+", " ", parsed.full_text or "").strip()
        global_summ = fallback_text[:150] if fallback_text else f"文档《{parsed.filename}》已完成分块入库。"
        print("[DocChunker] ⚠️ 使用文本片段作为摘要兜底")

    if not global_tags:
        filename_tags = [
            t for t in re.split(r"[\s_\-—–（）()【】\\/.]+", parsed.filename or "")
            if len(t.strip()) >= 2
        ]
        heading_tags = [
            (s.heading or "").strip()
        ...
        global_tags = list(dict.fromkeys([category, *filename_tags, *heading_tags]))[:8]
        print(f"[DocChunker] ⚠️ 使用文件名/标题作为标签兜底: {global_tags}")
