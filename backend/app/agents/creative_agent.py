...
# ---------------- 工具函数执行区 ----------------
def _rewrite_xhs_search_query(raw_query: str) -> str:
    """把 skill 任务句降噪成搜索词，不猜测用户没有写出的关键词。"""
    text = str(raw_query or "").strip()
    if not text:
        return ""

    explicit_match = re.search(
        r"(?:搜索小红书|小红书搜索|搜索|查找|找一下|找找)\s*(.+?)(?:[，。,.;；]|$)",
        text,
    )
    if explicit_match:
        keyword = explicit_match.group(1).strip()
        keyword = re.sub(r"\s+", "", keyword)
        if keyword:
            print(f"[CreativeAgent] 🔎 小红书搜索词改写: raw='{text}' -> keyword='{keyword}'")
            return keyword

    cleaned = re.sub(
        r"(帮我|请|麻烦|做一组|做一个|整理\s*\d+\s*个\w*|总结\s*\d+\s*个\w*|"
        r"每页放\s*\d+\s*个\w*|"
        r"用我选中的灵感笔记|用选中的灵感笔记|"
        r"生成小红书信息图|生成信息图|小红书信息图)",
        " ",
        text,
    )
...
async def _execute_analyze_inspiration(state: PlannerState, decision: dict):
    ...
    if decision.get("parameters", {}).get("autoXhsSearch") and not selected_ids:
        from app.services.xhs_service import search_xhs_livesearch
        user_id = extra_context.get("user_id")
        search_query = _rewrite_xhs_search_query(query)
        print(f"[CreativeAgent] 🔁 技能链路：先执行小红书搜索再分析，query='{query}', search_query='{search_query}', user_id={user_id}")
        search_res = await search_xhs_livesearch(
            search_query,
            limit=10,
            user_id=user_id,
            detail_limit=10,
        ...
        if search_res.get("status") == "precheck_failed":
            precheck = search_res.get("precheck") or {}
            reply = precheck.get("message") or search_res.get("message") or "小红书环境预检未通过。"
            message_content = json.dumps({
                "thought": decision.get("thought", "先检查小红书环境是否可用。"),
                "action": "run_xhs_search",
                "reply": reply,
            ...
            return {
                "final_decision": {**decision, "action": "run_xhs_search", "reply": reply, "xhs_precheck_failed": precheck},
                "messages": [AIMessage(content=message_content)]
            }
        if search_res.get("status") != "success":
            reply = search_res.get("message") or "小红书搜索失败，无法继续整理活动合集。"
            return {"final_decision": {**decision, "reply": reply}, "messages": [AIMessage(content=reply)]}
        ...
        if searched_ids:
            selected_ids = searched_ids
            print(f"[CreativeAgent] ✅ 技能链路：使用刚搜索入库的 {len(selected_ids)} 条笔记继续分析")
        else:
            reply = "小红书搜索没有返回可用笔记，无法继续整理活动合集。"
            return {"final_decision": {**decision, "reply": reply}, "messages": [AIMessage(content=reply)]}
    ...
    if not structured_items and isinstance(res, str) and res.strip():
        structured_items = [{
            "title": "活动合集草稿",
            "subtitle": "",
            "date": "",
...
async def _execute_copy_writer_llm(state: PlannerState, decision: dict):
    ...
    if decision.get("action") == "summary_draft":
        # 草稿箱正文必须使用可编辑正文，不使用固定提示语。
        draft_res = res
        if isinstance(draft_res, str) and "✅" in draft_res and ("提取" in draft_res or "完成" in draft_res):
            lines = draft_res.split('\n')
            if len(lines) > 1 and "✅" in lines[0]:
                draft_res = "\n".join(lines[1:]).strip()
        res = draft_res or content
