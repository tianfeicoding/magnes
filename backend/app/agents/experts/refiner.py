...
async def refiner_node(state: MagnesState):
    ...
    if analysis_res["status"] == "success":
        ...
        if blocks:
            ...
            if len(blocks) > 1:
                ...
                style_data = style_block.get("style", {})
                # 支持 bilingual 结构: prompt_text.zh / prompt_text.en
                prompt_text = style_data.get("prompt_text", {})
                prompt_text_zh = prompt_text.get("zh", "")
                prompt_text_en = prompt_text.get("en", "")
                # 兜底：兼容旧版 backgroundPrompt
                if not prompt_text_zh:
                    prompt_text_zh = style_data.get("backgroundPrompt", "")
                style_prompt = prompt_text_zh  # 保持兼容性

            # 如果没找到 Block B，但 Block A 包含风格（某些模型会混在一起）
            if not style_prompt:
                style_data = blocks[0].get("style", {})
                prompt_text = style_data.get("prompt_text", {})
                prompt_text_zh = prompt_text.get("zh", "")
                prompt_text_en = prompt_text.get("en", "")
                if not prompt_text_zh:
                    prompt_text_zh = style_data.get("backgroundPrompt", "")
                style_prompt = prompt_text_zh
    ...
    return {
        ...
        "style_prompt": style_prompt, # 精准提取的风格描述 (中文, 用于展示)
        "prompt_text_zh": prompt_text_zh,  # 中文提示词 (给用户看)
        "prompt_text_en": prompt_text_en,  # 英文提示词 (给生图模型用)
