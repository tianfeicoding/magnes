...
def _is_valid_stream_url(val) -> bool:
    """验证流式事件中的图片 URL 是否有效"""
...
async def handle_stream_events(planner_graph, input_data, config, message: str, active_image_url: Optional[str]) -> AsyncGenerator[dict, None]:
    ...
    async def pump_langgraph_events():
        nonlocal last_reply_content, sent_skill_recall
        try:
                ...
                if kind == "on_chat_model_stream":
                    ...
                    if decision:
                        if event_name == "LangGraph" or event_name in [
                            ...
                            if decision.get("skill_recall") and not sent_skill_recall:
                                sent_skill_recall = True
                                recall = decision["skill_recall"]
                                await emit({
                                    "type": "skill_recall",
                                    "skillId": recall.get("active_skill"),
                                    "skillName": recall.get("skill_name"),
                                    "activationMode": recall.get("activation_mode"),
                                    "confidence": recall.get("confidence"),
                                    "missingSlots": recall.get("missing_slots") or [],
                            ...
                            if current_action and current_action != "chat" and current_action not in sent_actions:
                                ...
                                # 防御：过滤掉无效的图片 URL（如 UserImage 占位符）
                                if params.get("imageUrl") and not _is_valid_stream_url(params.get("imageUrl")):
                                    print(f"[Streaming] ⚠️ 过滤无效 imageUrl: {params['imageUrl']}")
                                    params["imageUrl"] = active_image_url if _is_valid_stream_url(active_image_url) else None
                                if "image_urls" in params and isinstance(params["image_urls"], list):
                                    params["image_urls"] = [u for u in params["image_urls"] if _is_valid_stream_url(u)]
