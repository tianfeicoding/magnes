...
async def run_planner(
    ...
    **kwargs
    ...
    is_ecommerce_skill_active = active_skill == "ecommerce-image-gen"
    if active_image_url and is_ecommerce_skill_active and is_new_image and active_tab != 'xhs':
        yield {"type": "thought", "content": "识别到电商技能上传图，先同步商品图到画布。"}
        yield {"type": "action", "action": "mirror_image", "parameters": {"imageUrl": active_image_url}}

    if active_image_url and any(kw in (message or "") for kw in IMAGE_UPLOAD_KEYWORDS) and active_tab != 'xhs' and not is_ecommerce_skill_active:
        if has_template:
