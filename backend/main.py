...
from app.api.skill_routes import router as skill_router        # 技能库
...
async def ensure_lightweight_migrations():
    """补齐 create_all 不会自动添加的 SQLite 索引/约束。"""
    async with engine.begin() as conn:
        await conn.execute(text(
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_user_memories_user_type_key "
            "ON user_memories (user_id, memory_type, key)"
        ))
...
async def lifespan(app: FastAPI):
    ...
    await ensure_lightweight_migrations()
    print("📋 数据库表初始化完成")
    ...
    print("DEBUG: [Lifespan] 即将导入 RAG 模块...")
    ...
    print("DEBUG: [Lifespan] RAG 模块导入完成")
    try:
        # LlamaIndex 全局配置初始化
        print("DEBUG: [Lifespan] 开始调用 init_rag_settings()...")
        await init_rag_settings()
        print("DEBUG: [Lifespan] init_rag_settings() 完成")
...
async def create_design_task(request: DesignRequest):
    ...
    """
    image_preview = request.image_url or ""
    if len(image_preview) > 120:
        image_preview = f"{image_preview[:60]}...[truncated {len(request.image_url) - 100} chars]...{image_preview[-40:]}"
    print(
        "[Design API] 收到设计请求: "
        f"thread_id={request.thread_id}, "
        f"instruction={request.instruction}, "
        f"num_layers={request.num_layers}, "
        f"run_painter={request.run_painter}, "
        f"run_refiner={request.run_refiner}, "
        f"image_url={bool(request.image_url)} {image_preview}"
    )
    ...
    output = result.get("layout_schema") or {}
    ...
    if error or str(current_step or "").startswith("error_"):
        return {
            "thread_id": request.thread_id,
            "status": "error",
            "current_step": current_step,
            "message": error or current_step or "设计工作流失败",
            "output": output,
        }

    if request.image_url and not request.run_painter and not layers:
        return {
            "thread_id": request.thread_id,
            "status": "error",
            "current_step": current_step,
            "message": "图层拆解没有返回任何图层，请检查分层 API 配置或稍后重试。",
            "output": output,
        }
...
app.include_router(skill_router, prefix="/api/v1", dependencies=common_deps)    # 技能库
