...
(function () {
    ...
    API.Skills = {
        /**
         * 获取技能候选列表。默认 status=draft，用于技能草稿箱。
         */
        listCandidates: async ({ status } = {}) => {
            const query = status ? `?status=${encodeURIComponent(status)}` : '';
            const res = await API.magnesFetch(`skills/candidates${query}`, {
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`加载技能草稿失败: ${res.status}`);
            }
            return res.json();
        },
         ...
         * 从任务轨迹生成技能草稿。
         */
        createCandidateFromTrace: async ({ traceId, source = 'user_saved' }) => {
            const res = await API.magnesFetch('skills/candidates/from-trace', {
                method: 'POST',
                body: JSON.stringify({ traceId, source }),
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`创建技能草稿失败: ${res.status}`);
            }
            return res.json();
        },
         ...
         * 从当前画布状态生成技能草稿。
         */
        createCandidateFromCanvas: async ({ projectId, projectName, conversationId, nodes, edges, source = 'user_saved' }) => {
            const res = await API.magnesFetch('skills/candidates/from-canvas', {
                method: 'POST',
                body: JSON.stringify({
                    projectId,
                    projectName,
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.detail || `保存技能草稿失败: ${res.status}`);
            }
            return res.json();
        },
        ...
        updateCandidate: async (candidateId, data) => {
            const res = await API.magnesFetch(`skills/candidates/${candidateId}`, {
                method: 'PATCH',
                body: JSON.stringify(data),
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`更新技能草稿失败: ${res.status}`);
            }
            return res.json();
        },
         ...
         * 获取已生成的运行时技能。
         */
        listInstalled: async ({ enabled } = {}) => {
            const query = typeof enabled === 'boolean' ? `?enabled=${enabled ? 'true' : 'false'}` : '';
            const res = await API.magnesFetch(`skills/installed${query}`, {
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`加载我的技能失败: ${res.status}`);
            }
            return res.json();
        },

        /**
         * 使用已安装技能，返回 activeSkill 注入信息。
         */
        useInstalled: async (installedId) => {
            const res = await API.magnesFetch(`skills/installed/${installedId}/use`, {
                method: 'POST',
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`使用技能失败: ${res.status}`);
            }
            return res.json();
        },

        /**
         * 通过 runtime skill_id 使用已安装技能，用于对话内 Skill 标签等入口。
         */
        useInstalledBySkillId: async (skillId) => {
            const res = await API.magnesFetch(`skills/installed/by-skill/${encodeURIComponent(skillId)}/use`, {
                method: 'POST',
                triggerLogin: true
            });
            if (!res.ok) {
        ...
        /**
         * 更新已安装技能，例如启用/停用或更新 manifest。
         */
        updateInstalled: async (installedId, data) => {
            const res = await API.magnesFetch(`skills/installed/${installedId}`, {
                method: 'PATCH',
                body: JSON.stringify(data),
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`更新技能失败: ${res.status}`);
            }
            return res.json();
        },
         ...
         * 获取技能运行记录。
         */
        listRuns: async (installedId, { limit = 30 } = {}) => {
            const res = await API.magnesFetch(`skills/installed/${installedId}/runs?limit=${limit}`, {
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`加载技能运行记录失败: ${res.status}`);
            }
            return res.json();
        },
         ...
         * 获取技能更新建议。
         */
        listPatchProposals: async ({ status = 'pending', skillId } = {}) => {
            const params = new URLSearchParams();
            if (status) params.set('status', status);
            if (skillId) params.set('skill_id', skillId);
            const query = params.toString() ? `?${params.toString()}` : '';
            const res = await API.magnesFetch(`skills/patch-proposals${query}`, {
                triggerLogin: true
            });
            if (!res.ok) {
                throw new Error(`加载技能更新建议失败: ${res.status}`);
            }
            return res.json();
        },
         ...
         * 应用或忽略技能更新建议。
         */
        updatePatchProposal: async (proposalId, status) => {
            const res = await API.magnesFetch(`skills/patch-proposals/${proposalId}`, {
                method: 'PATCH',
            ...
            if (!res.ok) {
                const errorData = await res.json().catch(() => ({}));
                throw new Error(errorData.detail || `更新技能建议失败: ${res.status}`);
            }
            return res.json();
        }
    };
