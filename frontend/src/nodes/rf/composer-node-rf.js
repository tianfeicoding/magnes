    ...
    const ComposerNode = ({ id, data, selected, nodesMap, connections: studioConnections }) => {
        ...
        React.useEffect(() => {
            ...
            setNodes(nds => nds.map(n => {
                if (n.id !== id) return n;
                // 只有当 content 真正变化时才更新，避免无限循环；不能只比数量，bbox/url 改变也必须同步。
                const currentSignature = JSON.stringify((n.data?.content?.layers || []).map(l => ({
                    id: l.id,
                    url: l.url || l.content || '',
                    bbox: l.bbox,
                    z_index: l.z_index,
                ...
                const nextSignature = JSON.stringify((compositeData.finalSchema.layers || []).map(l => ({
                    id: l.id,
                    url: l.url || l.content || '',
                    bbox: l.bbox,
                    z_index: l.z_index,
