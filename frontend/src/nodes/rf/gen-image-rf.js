...
(function () {
    ...
    const isValidImgUrl = (u) => {
        ...
        const bad = ['placeholder', 'userimage', 'reuse_context_image', 'none'];
    ...
    const GenImageNode = ({ id, data, selected }) => {
        ...
        const getGroupedImages = useCallback(async () => {
            ...
            if (latestData.settings?.sourceImages && Array.isArray(latestData.settings.sourceImages)) {
                finalImages.push(...latestData.settings.sourceImages.filter(isValidImgUrl));
            }
            ...
            for (const sourceId of sourceMap.keys()) {
                ...
                if (sourceNode.type === 'layer-split') {
                    ...
                    const visibleUrls = layers
                        .filter((l, idx) => !hiddenIdx.includes(idx) && l.url && isValidImgUrl(l.url))
                        .map(l => l.url);
                ...
                } else if (sourceNode.data?.content && isValidImgUrl(sourceNode.data.content)) {
        ...
        const getConnectedImages = useCallback(() => {
            ...
            if (data.settings?.sourceImages && Array.isArray(data.settings.sourceImages)) {
                previewImages.push(...data.settings.sourceImages.filter(isValidImgUrl));
            }
            ...
            for (const sourceId of sourceMap.keys()) {
                ...
                if (sourceNode.type === 'layer-split') {
                    const layers = sourceNode.data?.layers || [];
                    const activeLayer = layers.find((l, idx) => !(sourceNode.data?.hiddenLayers || []).includes(idx) && l.url && isValidImgUrl(l.url));
