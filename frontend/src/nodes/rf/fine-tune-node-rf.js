    ...
    const FineTuneNode = ({ id, data, selected }) => {
        ...
        const handleSaveTemplate = async () => {
            ...
            if (SemanticService) {
                ...
                try {
                    const semanticTimeout = new Promise((_, reject) => {
                        setTimeout(() => reject(new Error('语义分析超时，改用当前图层直接保存')), 15000);
                    });
                    finalLayers = await Promise.race([
                        SemanticService.analyze(currentLayers),
                        semanticTimeout
                    ]);
                } catch (err) {
            ...
            try {
                ...
                if (res.ok) {
                    const suffix = semanticFallback
                        ? '\n\n注意：语义分析失败或超时，已按当前图层直接保存。后续可在模板中手动校正语义字段。'
                        : '';
                    alert(`✨ 模版保存成功！\n您现在可以在“图文模版”节点中看到并直接应用这个方案了。${suffix}`);
                } else {
        ...
        const isExplicitBackgroundLayer = (layer) => {
        ...
        // 背景设置使用独立字段，不再从内容图层推断，避免新精细编排继承上一次背景。
        const currentBgUrl = data.backgroundUrl || '';
        ...
        const handleLocalUpload = (e) => {
            ...
            reader.onload = (ev) => {
                ...
                if (bgLayerIdx >= 0) {
                    ...
                    saveToHistory(newLayers);
                    setNodes(nds => nds.map(n => n.id === id ? {
                        ...n, data: { ...n.data, isDirty: true, backgroundUrl: url, content: { ...schema, layers: newLayers } }
                    } : n));
                }
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    if (bgLayerIdx >= 0) {
                        ...
                        setNodes(nds => nds.map(n => n.id === id ? {
                            ...n,
                            data: { ...n.data, backgroundUrl: result.url }
                        } : n));
                    } else {
                        const newLayers = [...layers];
                        newLayers.push({
                        ...
                        saveToHistory(newLayers);
                        setNodes(nds => nds.map(n => n.id === id ? {
                            ...n, data: { ...n.data, isDirty: true, backgroundUrl: result.url, content: { ...schema, layers: newLayers } }
                        } : n));
                    }
