...
(function () {
    ...
    const MessageBubble = ({ msg, apiEndpoint, onSendMessage, setMessages, messages, msgIndex }) => {
        ...
        const isPendingWorkflowMessage = /^正在/.test(displayContent) || /正在.*(搜索|整理|生成|处理|执行)/.test(displayContent);

        const [isModalOpen, setIsModalOpen] = useState(false);

        const handlePatchProposal = async (status) => {
            const proposalId = msg.skillPatchProposal?.id;
            if (!proposalId || patchBusy) return;
            setPatchBusy(true);
            try {
                const API = window.MagnesComponents.Utils.API;
                const result = await API.Skills.updatePatchProposal(proposalId, status);
                ...
                window.dispatchEvent(new CustomEvent('magnes:toast', {
                    detail: {
                        message: status === 'applied' ? '技能已更新' : '已忽略技能更新建议',
                        type: 'success'
                    }
                }));
                if (window.MagnesComponents?.UI?.toast) {
                    window.MagnesComponents.UI.toast(status === 'applied' ? '技能已更新' : '已忽略技能更新建议', 'success');
                }
            } catch (err) {
                window.dispatchEvent(new CustomEvent('magnes:toast', {
                    detail: {
                        message: err.message || '更新技能建议失败',
                        type: 'error'
                    }
                }));
                if (window.MagnesComponents?.UI?.toast) {
                    window.MagnesComponents.UI.toast(err.message || '更新技能建议失败', 'error');
                }
            } finally {
                setPatchBusy(false);
            }
        ...
        return (
            <div className={`flex flex-col gap-1 ${isUser ? 'items-end' : 'items-start'}`}>
                ...
                {!isUser && msg.skillRecall && (
                    <div className="max-w-[240px] border border-black bg-zinc-50 px-2 py-1 text-[10px] text-black">
                        <span className="font-bold">已召回技能：</span>
                        <span>{msg.skillRecall.skillName || msg.skillRecall.skillId}</span>
                        {typeof msg.skillRecall.confidence === 'number' && (
                            <span className="text-zinc-500"> · {Math.round(msg.skillRecall.confidence * 100)}%</span>
                        )}
                    </div>
                )}

                {!isUser && msg.skillPatchProposal && (
                    <div className="max-w-[240px] border border-black bg-white text-black">
                        <div className="px-3 py-2 border-b border-black text-[10px] font-bold uppercase tracking-widest">
                            技能更新建议
                        </div>
                        <div className="px-3 py-2 text-[12px] leading-relaxed">
                            <div className="font-bold mb-1">检测到一条可沉淀规则</div>
                            <div className="text-zinc-600">
                                {msg.skillPatchProposal.issueSummary || msg.skillPatchProposal.message || '是否把这次纠正更新到技能？'}
                            </div>
                        </div>
                        {msg.skillPatchProposal.status === 'applied' || msg.skillPatchProposal.status === 'ignored' ? (
                            <div className="px-3 py-2 border-t border-zinc-200 text-[11px] font-bold">
                                {msg.skillPatchProposal.status === 'applied' ? '已更新到技能' : '已忽略'}
                            </div>
                        ) : (
                            <div className="px-3 py-2 border-t border-zinc-200 flex gap-2">
                                <button
                                    onClick={() => handlePatchProposal('applied')}
                                    disabled={patchBusy}
                                    className="px-2 py-1 bg-black text-white text-[11px] font-bold hover:bg-zinc-800 disabled:opacity-40"
                                >
                                    更新技能
                                </button>
                                <button
                                    onClick={() => handlePatchProposal('ignored')}
                                    disabled={patchBusy}
                                    className="px-2 py-1 bg-white border border-black text-black text-[11px] font-bold hover:bg-zinc-50 disabled:opacity-40"
                                >
                                    忽略
                                </button>
                            </div>
                ...
                {msg.content && (
                    <div className="flex flex-col gap-2 items-start">
                        ...
                        {!isUser && !apiEndpoint.includes('rag') && !msg.isGenerating &&
                            !isPendingWorkflowMessage && !displayContent.includes('正在为您搜索') && msg.action !== 'run_xhs_search' &&
                                    ...
                                    {((msg.final_decision?.results?.length > 0) || (msg.sourceIds?.length > 0) || msg.action === 'summary_draft' || msg.action === 'analyze_inspiration') && (
                                        <>
                                            {!apiEndpoint.includes('rag') && (
                                                <button
                                                    onClick={() => {
                                                        // 草稿箱应编辑助手生成的答案；用户问题只作为触发语，不应进入草稿正文。
                                                        let draftContent = msg.parameters?.raw_draft_content || msg.parameters?.text_to_check || msg.final_decision?.reply || msg.reply || displayContent || msg.content || '';
                                                        if (typeof draftContent !== 'string') draftContent = JSON.stringify(draftContent, null, 2);
                                                        draftContent = draftContent.replace(/\\n/g, '\n');
                                                        if (/^✅\s*活动信息提取完成/.test(draftContent.trim())) {
                                                            draftContent = msg.parameters?.text_to_check || msg.parameters?.content || displayContent || msg.content || '';
                                                            if (typeof draftContent !== 'string') draftContent = JSON.stringify(draftContent, null, 2);
                                                            draftContent = draftContent.replace(/\\n/g, '\n');
                                                        }
                                            ...
                                            <button
                                                onClick={() => {
                                                    ...
                                                    const docIds = Array.from(new Set([...(msg.sourceIds || []), ...ids]));
    ...
    const ConversationPanel = ({
        ...
        const [runtimeSkill, setRuntimeSkill] = useState(activeSkill || null);
        const [runtimeSkillSummary, setRuntimeSkillSummary] = useState(skillSummary || null);
        ...
        useEffect(() => {
            if (activeSkill) setRuntimeSkill(activeSkill);
            if (skillSummary) setRuntimeSkillSummary(skillSummary);
        }, [activeSkill, skillSummary]);
        ...
        const sendMessage = useCallback(async (text, imageUrl = null, options = {}) => {
            ...
            try {
                ...
                const effectiveActiveSkill = options.activeSkill || runtimeSkill || activeSkill || null;
                const effectiveSkillSummary = options.skillSummary || runtimeSkillSummary || skillSummary || null;
                const response = await API.magnesFetch(cleanEndpoint, {
                    ...
                    body: JSON.stringify({
                        ...
                        history,
                        canvasContext,
                        activeSkill: effectiveActiveSkill,
                        skillSummary: effectiveSkillSummary,
                ...
                const processBuffer = (targetBuffer) => {
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            if (event.type === 'skill_recall') {
                                setRuntimeSkill(event.skillId || runtimeSkill);
                                setRuntimeSkillSummary(event.skillName || runtimeSkillSummary);
                                setMessages(prev => prev.map(m =>
                                    m.id === currentAiMsgId ? {
                                        ...m,
                                        skillRecall: {
                            ...
                            } else if (event.type === 'skill_patch_proposal') {
                                const proposal = event.proposal || {};
                                setMessages(prev => prev.map(m =>
                                    m.id === currentAiMsgId ? {
                                        ...m,
                                        content: event.message || m.content || '检测到技能更新建议。',
                                        skillPatchProposal: {
                                            ...proposal,
                                ...
                                if (ragIds.length > 0) {
                                    setMessages(prev => prev.map(m =>
                                        m.id === currentAiMsgId ? { ...m, sourceIds: Array.from(new Set([...(m.sourceIds || []), ...ragIds])) } : m
                                    ));
                                }
                                ...
                                setMessages(prev => prev.map(m =>
                                    m.id === currentAiMsgId ? { ...m, sourceIds: Array.from(new Set([...(m.sourceIds || []), ...ids])), results: event.results } : m
                                ));
        ...
        }, [isGenerating, conversationId, activeSkill, skillSummary, runtimeSkill, runtimeSkillSummary, getNodes, mirrorImageToCanvas, handlePlannerAction, apiEndpoint, onRetrievalResults, onRewrittenQueries]);

        // 清理 SSE 连接 & 处理 Skill 激活
        useEffect(() => {
            const handleSkillActivate = async (e) => {
                const { id, label, summary, starterPrompt, resolved } = e.detail || {};
                let activeId = id;
                let activeLabel = label;
                let activeSummary = summary;
                let activeStarterPrompt = starterPrompt;

                if (!resolved && id) {
                    try {
                        const API = window.MagnesComponents.Utils.API;
                        const result = await API.Skills.useInstalledBySkillId(id);
                        const data = result?.data || {};
                        activeId = data.activeSkill || id;
                        activeLabel = data.skill?.name || label || activeId;
                        activeSummary = data.skillSummary || summary || activeLabel;
                        activeStarterPrompt = data.starterPrompt || starterPrompt;
                    } catch (err) {
                        console.warn('[ConversationPanel] 使用 installed skill 失败，回退到直接激活:', err);
                    }
                }

                console.log(`[ConversationPanel] 🚀 激活 Skill: ${activeLabel} (${activeId})`);
                setRuntimeSkill(activeId);
                setRuntimeSkillSummary(activeSummary || activeLabel || activeId);

                const skillLabel = activeLabel || activeId;
                const activationPrompt = activeStarterPrompt
                    ? `已激活${skillLabel}。请按下方示例补充任务信息后发送。`
                    : `已激活${skillLabel}。请继续输入要执行的任务。`;
                setMessages(prev => [...prev, {
                    id: `skill_activation_${Date.now()}`,
                    role: 'assistant',
                    content: activationPrompt,
                    timestamp: new Date(),
                    action: 'chat'
                }]);
                if (activeStarterPrompt) {
                    setInputText(activeStarterPrompt);
                }

                // 只更新本地 activeSkill；用户真正发送任务时再随请求注入后端。
            };
