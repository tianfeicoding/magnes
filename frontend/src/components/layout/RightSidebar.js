            ...
            <div className={`${sidebarCollapsed ? 'w-8' : 'w-[320px]'} border-l border-black flex flex-col bg-white shrink-0 relative overflow-hidden transition-all duration-300`}>
                {isAssetsTab ? (
                    <div className="flex flex-col h-full bg-white">
                        {/* Assets Header */}
                        <div className="px-4 py-2 border-b border-black flex items-center gap-2 bg-white shrink-0">
                            <button
                        ...
                        <ConversationPanel
                            ...
                            onTriggerGeneration={async (action, params) => {
                                ...
                                if (action === 'run_painter') {
                                    ...
                                    // 防御：过滤无效的图片 URL（如 UserImage 占位符）
                                    ...
                                    const rawUrls = params.image_urls || (params.imageUrl ? [params.imageUrl] : []);
                                    const image_urls = rawUrls
                                        .map(u => u === 'REUSE_CONTEXT_IMAGE' ? lastUrl : u)
