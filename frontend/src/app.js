(function () {
    ...
    const { SkillLibraryPanel } = window.MagnesComponents.Skills || {};
    ...
    const MagnesStudioContent = () => {
        ...
        const handleNewProject = useCallback(async () => {
            ...
            localStorage.setItem('magnes_conversation_id', newConversationId);
            setActiveTab('canvas');
            toast('✨ 新项目已创建', 'success');
        }, [isLoggedIn, setNodes, setEdges, setMessages, setConversationId, toast]);

        const handleSaveCanvasAsSkillDraft = useCallback(async () => {
            if (!isLoggedIn) {
                setLoginModalOpen(true);
                return;
            }
            if (!nodes.length) {
                toast('当前画布没有节点，无法保存技能草稿', 'warning');
                return;
            }
            ...
            try {
                const result = await API.Skills.createCandidateFromCanvas({
                ...
                if (result?.status !== 'success') {
                    throw new Error(result?.detail || '保存技能草稿失败');
                }
                toast('已保存到技能草稿箱', 'success');
                setActiveTab('skills');
            } catch (e) {
                console.error('[Magnes] ❌ 保存画布技能草稿失败:', e);
                toast(e.message || '保存技能草稿失败', 'error');
            } finally {
                setIsSavingSkillDraft(false);
            }
        }, [isLoggedIn, nodes, edges, currentProjectId, projectName, conversationId, setActiveTab, toast]);
        ...
        return (
            <div className={`w-full h-full flex flex-col ${theme === 'dark' ? 'dark' : ''}`}>
                ...
                <AppHeader
                    ...
                    onSaveSkillDraft={handleSaveCanvasAsSkillDraft}
                    isSavingSkillDraft={isSavingSkillDraft}
                    // 用户认证相关
                    user={user}
                ...
                <div className="flex-1 overflow-hidden relative flex">
                    ...
                    <div className="flex-1 h-full relative overflow-hidden flex flex-col min-w-0" style={{ backgroundColor: '#ffffff' }}>
                        {activeTab === 'my-projects' ? (
                            ...
                            React.createElement(SkillLibraryPanel, { toast, setActiveTab })
                        ) : (activeTab === 'canvas' || activeTab === 'assets') ? (
                            <MagnesCanvas
                                nodes={nodes}
                                edges={edges}
