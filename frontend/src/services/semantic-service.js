...
(function () {
    ...
    class SemanticService {
        ...
        /**
         * LLM 语义反演后处理。
         * 只在检测到明确的重复活动组结构、且某个 group 缺 title 时，才把紧邻其上方的标题层归组。
         * 这样可以修复“第一个活动标题被误判为 main_title”的问题，同时保留真实全局主标题。
         */
        repairGroupedTitles(layers) {
            const Helpers = window.MagnesComponents?.Utils?.ParseHelpers;
            const normalizeRole = (role) => Helpers ? Helpers.normalizeRole(role) : role;
            const bboxOf = (layer) => {
                const bbox = layer?.bbox || [0, 0, 0, 0];
                if (Array.isArray(bbox)) return bbox;
                return [bbox.x || 0, bbox.y || 0, bbox.width || bbox.w || 0, bbox.height || bbox.h || 0];
            };
            const fontSizeOf = (layer) => parseFloat(layer?.style?.fontSize || 0) || 0;
            const isSingleLineTitleLike = (layer) => {
                const text = String(layer?.text || layer?.content || '').trim();
                if (!text || text.includes('\n')) return false;
                if (text.length > 42) return false;
                if (/^(时间|日期|地点|场地|地址|门票|价格|票价|费用|亮点|特色|介绍|内容)\s*[:：]/.test(text)) return false;
                return true;
            };

            const textLayers = layers.filter(l => l.type === 'text' || l.type === 'body');
            const grouped = {};
            textLayers.forEach(layer => {
                const match = String(layer.groupId || '').match(/^group_(\d+)$/);
                if (!match) return;
                const groupId = `group_${match[1]}`;
                grouped[groupId] = grouped[groupId] || [];
                grouped[groupId].push(layer);
            });

            const groupIds = Object.keys(grouped).sort((a, b) => {
                return parseInt(a.replace('group_', ''), 10) - parseInt(b.replace('group_', ''), 10);
            });
            if (groupIds.length < 2) return layers;

            const titleGroups = groupIds.filter(groupId =>
                grouped[groupId].some(layer => normalizeRole(layer.semanticRole || layer.role) === 'title')
            );
            if (titleGroups.length === 0) return layers;

            const repairedById = {};

            groupIds.forEach(groupId => {
                const groupLayers = grouped[groupId] || [];
                const hasTitle = groupLayers.some(layer => normalizeRole(layer.semanticRole || layer.role) === 'title');
                if (hasTitle) return;

                const peerTitleLayers = titleGroups
                    .filter(peerGroupId => peerGroupId !== groupId)
                    .flatMap(peerGroupId => grouped[peerGroupId] || [])
                    .filter(layer => normalizeRole(layer.semanticRole || layer.role) === 'title');
                if (peerTitleLayers.length === 0) return;

                const groupBoxes = groupLayers.map(bboxOf);
                const groupTop = Math.min(...groupBoxes.map(b => b[1]));
                const groupLeft = Math.min(...groupBoxes.map(b => b[0]));
                const groupFontMax = Math.max(...groupLayers.map(fontSizeOf), 0);

                const candidates = textLayers
                    .filter(layer => !/^group_\d+$/.test(String(layer.groupId || '')))
                    .filter(isSingleLineTitleLike)
                    .map(layer => {
                        const bbox = bboxOf(layer);
                        const bottom = bbox[1] + bbox[3];
                        const verticalGap = groupTop - bottom;
                        const xDelta = Math.abs(bbox[0] - groupLeft);
                        const role = normalizeRole(layer.semanticRole || layer.role);
                        const fontSize = fontSizeOf(layer);
                        return { layer, bbox, verticalGap, xDelta, role, fontSize };
                    })
                    .filter(c => c.verticalGap >= -10 && c.verticalGap <= 140)
                    .filter(c => c.xDelta <= 140)
                    .filter(c => c.role === 'title' || c.role === 'main_title' || c.role === 'main' || c.role === 'other' || c.role === 'text')
                    .filter(c => c.fontSize === 0 || groupFontMax === 0 || c.fontSize >= groupFontMax)
                    .filter(c => peerTitleLayers.some(peer => {
                        const peerBox = bboxOf(peer);
                        const peerFont = fontSizeOf(peer);
                        const fontClose = !c.fontSize || !peerFont || Math.abs(c.fontSize - peerFont) <= Math.max(8, peerFont * 0.25);
                        const xAligned = Math.abs(c.bbox[0] - peerBox[0]) <= 140;
                        return fontClose && xAligned;
                    }))
                    .sort((a, b) => Math.abs(a.verticalGap) - Math.abs(b.verticalGap));

                const best = candidates[0];
                if (!best) return;

                repairedById[best.layer.id] = {
                    ...best.layer,
                    role: 'title',
                    semanticRole: 'title',
                    groupId
                };
                console.log(`[SemanticService] Repaired grouped title: ${best.layer.id} -> ${groupId}`);
            });

            if (Object.keys(repairedById).length === 0) return layers;
            return layers.map(layer => repairedById[layer.id] || layer);
        }
        ...
        async analyze(layers) {
            ...
            try {
                ...
                const repairedLayers = this.repairGroupedTitles(newLayers);
                console.log(`[SemanticService] Analysis Success (${repairedLayers.length} layers): `, repairedLayers);
