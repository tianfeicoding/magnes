...
(function () {
    ...
    function SourceModal({ isOpen, onClose, docIds, sourceMap, content, toast }) {
        ...
        const sourceDetails = useMemo(() => {
            ...
            const cleanActivityTitle = (value) => value
            ...
            const attachCitations = (line, sectionName, currentResults) => {
                citationRegex.lastIndex = 0;
                let match;
                let foundCitation = false;
                const name = sectionName || '参考信息';
                let section = currentResults.find(r => r.section === name);

                while ((match = citationRegex.exec(line)) !== null) {
                    foundCitation = true;
                    const rawNote = match[1];
                    const rawDetail = match[2] || '';
                    const noteNums = rawNote.match(/\d+/g) || [];
                    if (noteNums.length === 0) continue;

                    const details = rawDetail.split(/[,，\s]+/).filter(d => d.trim());
                    if (details.length === 0) details.push('正文');

                    if (!section) {
                        section = { section: name, noteMap: {} };
            ...
            lines.forEach(line => {
                ...
                const lineWithoutCitations = cleanActivityTitle(trimmed);
                const hasCitation = citationRegex.test(trimmed);
                citationRegex.lastIndex = 0;
                const isDetailLine = /^(时间|地点|门票|票价|亮点|日期|地址|费用|价格|展期|开放时间)\s*[:：]/.test(lineWithoutCitations);

                // 识别活动标题：不再排除数字 (解决 "搞事市集vol.7" 问题)
                // 逻辑：首行、加粗或较短行，且不含冒号（冒号通常引导详情）
                const isTitleCandidate = lineWithoutCitations &&
                    !isDetailLine &&
                    !lineWithoutCitations.includes('【') &&
                    (
                        hasCitation ||
                        (/^\s*(\*\*|#|•|-|\d+[.、])?\s*.{2,60}?\s*(\*\*)?\s*$/.test(lineWithoutCitations) &&
                            !lineWithoutCitations.includes(':') && !lineWithoutCitations.includes('：'))
                    );

                if (isTitleCandidate) {
                    currentActivity = lineWithoutCitations;
                    ...
                    attachCitations(trimmed, currentActivity, currentResults);
                    return;
                }
