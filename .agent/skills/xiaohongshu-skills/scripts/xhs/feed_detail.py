...
_EXTRACT_DETAIL_DOM_FALLBACK_JS = """
(() => {
    const text = (selector) => {
    ...
    const imgs = Array.from(document.querySelectorAll("img"))
        ...
        .filter((src) => src && /xhscdn|sns-webpic|notes_pre|webp|jpg|jpeg|png/i.test(src))
        .slice(0, 12)
        .map((src) => ({ urlDefault: src }));
...
def _extract_feed_detail(page: Page, feed_id: str) -> FeedDetailResponse:
    ...
    fallback = page.evaluate(_EXTRACT_DETAIL_DOM_FALLBACK_JS)
    if fallback:
        fallback_data = json.loads(fallback)
        note_data = fallback_data.get("note") or {}
        note_data["noteId"] = note_data.get("noteId") or feed_id
        return FeedDetailResponse(
            note=FeedDetail.from_dict(note_data),
