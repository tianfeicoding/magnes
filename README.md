...
| 🤖 **中心化多智能体** | Supervisor + Designer + Creative + Knowledge 4 大 Agent 调度，9 大 Expert 执行 |
| 🔄 **图片风格演化迭代** | 参考图上传→美学基因提取→LLM驱动生成与评分→用户触发迭代，V0→Vn版本历史可追溯 |
...
API 服务层 (FastAPI + Uvicorn)
  ...
  ├── /api/v1/dialogue   → Supervisor Agent 对话 SSE 流
...
智能体层 (LangGraph)
  ...
  └── Supervisor 对话图: Supervisor → DesignerAgent / CreativeAgent / KnowledgeAgent
         │
         ▼
核心服务层
...
│       │   ├── planner/        # Supervisor Agent（意图解析、路由分发）
│       │   │   ├── router.py
│       │   │   ├── graph.py
│       │   │   └── state.py
...
用户对话输入
    │
    ▼
supervisor_agent           ← LLM 解析用户意图，注入 Soul.md 记忆，输出 action
