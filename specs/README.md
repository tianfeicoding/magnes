...
- 🔍 **笔记灵感库**：搜索并聚合真实小红书笔记，基础笔记即时入库，AI 提取洞察并标注灵感来源
- 📋 **图文模版系统**：内置多种小红书排版风格，支持批量套用，可保存自定义模版
- 💾 **项目持久化**：画布状态（nodes/edges/viewport）自动保存，支持多项目管理与快照恢复
- 🧠 **长短期记忆系统**：Soul.md / MEMORY.md / ConversationSummary / MemoryEvent 分层存储，支持偏好注入、短期压缩和行为沉淀
- 🧩 **迭代技能库**：支持我的技能、技能草稿箱、启用生成 SKILL.md、自然语言自动召回、运行 trace、反馈补丁和版本迭代
...
- 项目持久化实体：Project、ProjectSnapshot
  - 记忆系统实体：UserMemory、CanvasActionLog、ConversationSummary、MemoryEvent、TaskTrace
  - 技能系统实体：SkillCandidate、InstalledSkill、SkillRun、SkillFeedbackEvent、SkillPatchProposal
- 关系模型与 ER 图
...
- 接口列表与调用关系
  ...
  - `GET/POST/PATCH /api/v1/skills/*`：技能草稿、启用技能、技能补丁建议
- 调用流程与序列图
...
- 🧠 **AI 长期记忆系统**
  ...
  - 短期会话压缩：按 token 阈值、任务阶段完成、工具链完成、项目/技能切换触发 ConversationSummary，保留最近原文与结构化摘要
  - 行为事件底座：MemoryEvent + TaskTrace 记录会话、工具、画布和反馈，为记忆回流与技能迭代提供证据

- 🧩 **迭代技能库**
  - 技能库 Tab：包含我的技能、技能草稿箱和在线技能库；技能补丁建议在对话卡片和补丁建议接口中呈现
  - 用户可从当前画布保存工作流为技能草稿，启用后生成 `.agent/skills/user-generated/*/SKILL.md` 和 `skill.json`
  - Planner 支持 activeSkill 显式触发和 enabled skill index 自然语言自动召回
  - SkillRun / SkillFeedbackEvent / SkillPatchProposal 支持运行追踪、用户纠正、补丁建议、确认后更新 SKILL.md、skill.json、CHANGELOG 和版本号
...
- 🔍 **笔记灵感库**
  - 搜索并聚合真实小红书笔记，搜索列表基础信息即时写入灵感库
  - 详情增强为可配置能力，用于补全文案、图片列表和互动元数据
  - AI 提取洞察并标注灵感来源
