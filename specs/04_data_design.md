...
### 2.12 ConversationSummary（对话摘要）

SQLAlchemy ORM 模型，存储于 SQLite `conversation_summaries` 表。用于长对话压缩与上下文管理（Phase 2 扩展）。
...
### 2.13 CanvasActionLog（画布操作日志）

SQLAlchemy ORM 模型，存储于 SQLite `canvas_action_logs` 表。记录用户在画布上的关键操作，用于后续行为分析与操作回溯（Phase 2 扩展）。
