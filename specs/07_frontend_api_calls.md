...
| `SkillLibraryPanel.js: listCandidates` | `/api/v1/skills/candidates` | GET | 获取技能草稿箱列表 |
| `app.js: saveCanvasAsSkillDraft` | `/api/v1/skills/candidates/from-canvas` | POST | 将当前画布保存为技能草稿 |
| `SkillLibraryPanel.js: updateCandidate` | `/api/v1/skills/candidates/{id}` | PATCH | 更新技能草稿名称、触发示例、步骤和 SKILL.md |
| `SkillLibraryPanel.js: installCandidate` | `/api/v1/skills/candidates/{id}/install` | POST | 启用技能草稿并生成 SKILL.md / skill.json |
| `SkillLibraryPanel.js: listInstalled` | `/api/v1/skills/installed` | GET | 获取已启用/已安装技能 |
| `SkillLibraryPanel.js: updateInstalled` | `/api/v1/skills/installed/{id}` | PATCH | 启停技能或更新 manifest |
| `conversation-panel.js: activeSkill` | `/api/v1/dialogue/run` | POST (SSE) | 对话中传入 activeSkill，后端加载对应 SKILL.md |
| `SkillLibraryPanel.js: listPatchProposals` | `/api/v1/skills/patch-proposals` | GET | 获取技能补丁建议 |
| `SkillLibraryPanel.js: updatePatchProposal` | `/api/v1/skills/patch-proposals/{id}` | PATCH | 应用/忽略技能补丁建议 |
...
- **触发时机**：用户打开「技能库」Tab，或切换到「技能草稿箱」列表。
- **接口**：`GET /api/v1/skills/candidates?status=draft`
- **响应**：
```json
{
  "status": "success",
  "data": [
    {
      "id": "candidate_xxx",
      "proposedName": "小红书活动合集工作流",
      "triggerExamples": ["搜索小红书上海五月热门市集，总结9个，粉色风格模版"],
      "workflowSteps": [{"index": 1, "phase": "dialogue", "label": "搜索小红书并同步灵感库"}],
      "requiredTools": ["rednote_workflow", "canvas_editing"],
      "draftSkillMd": "# 小红书活动合集工作流技能草稿\n...",
      "status": "draft"
    }
  ]
}
...
- **触发时机**：用户在画布页点击「保存为技能草稿」。
- **接口**：`POST /api/v1/skills/candidates/from-canvas`
- **请求体**：
```json
{
  "projectId": "proj_xxx",
  "projectName": "小红书活动合集工作流",
...
```
- **后端行为**：
  - 从 nodes/edges 提取画布工作流步骤。
  - 从 conversationId 查找最近 TaskTrace，补充“搜索小红书、总结活动、选择模板”等对话侧步骤。
  - 自动生成 trigger_examples、required_tools、draftSkillMd。

**更新技能草稿**

- **触发时机**：用户编辑草稿名称、触发示例、步骤或 SKILL.md。
- **接口**：`PATCH /api/v1/skills/candidates/{candidate_id}`
- **请求体**：
```json
{
  "proposedName": "小红书活动合集工作流",
...
- **触发时机**：用户点击「启用草稿」。
- **接口**：`POST /api/v1/skills/candidates/{candidate_id}/install`
- **后端行为**：
  - 写入 `.agent/skills/user-generated/{slug}-{candidate_id}/SKILL.md`。
  - 写入 `.agent/skills/user-generated/{slug}-{candidate_id}/skill.json`。
  - upsert `installed_skills`，设置 `enabled=1`、`version=1.0.0`。
- **响应**：返回 InstalledSkill 对象。

**获取我的技能**

- **触发时机**：技能库「我的技能」列表加载，或对话启动时构建 enabled skill index。
- **接口**：`GET /api/v1/skills/installed?enabled=true`
- **响应字段重点**：
  - `skillId`：运行时 ID。
  - `manifest`：包含 `recall_rules`、`required_slots`、`auto_run_policy`、`workflow_steps`、`version`。
  - `installPath`：相对 `.agent/skills/` 的路径。

**在对话中使用技能**

- **触发时机**：用户点击「使用技能」或自然语言命中已启用技能。
- **接口**：`POST /api/v1/dialogue/run`
- **请求体扩展**：
```json
{
  "message": "搜索小红书上海五月热门市集，总结9个，粉色风格模版",
  "conversationId": "conv_xxx",
  "activeSkill": "user-generated/小红书活动合集工作流-0af1f25d"
}
```
- **后端行为**：
  - 根据 activeSkill 或自动召回结果加载 manifest。
  - 读取对应 SKILL.md 并注入 Planner prompt。
  - 记录 SkillRun，后续工具调用和画布变化归因到该技能。

**技能补丁建议**

- **触发时机**：技能运行失败、用户纠正、用户反馈“不是这个模板/数量不对/来源不对”等。
- **接口**：`GET /api/v1/skills/patch-proposals?status=pending`
- **应用/忽略接口**：`PATCH /api/v1/skills/patch-proposals/{proposal_id}`
- **请求体**：
```json
{
  "status": "applied"
...
```
- **应用后的后端行为**：
  - SKILL.md 追加 `Learned Updates`。
  - skill.json 更新 `version` 与 `learned_updates`。
  - CHANGELOG.md 追加版本说明。
  - `installed_skills.version` 更新到新版本。
