...
| FR-15 | 功能 | 支持小红书搜索功能，用户可通过对话触发小红书笔记搜索、基础信息即时入库、灵感分析和活动总结生成。 |
...
| FR-MEM-05 | 功能 | 支持短期会话记忆压缩：基于 token 估算、工具链完成、任务阶段完成、项目/技能切换触发 ConversationSummary，保留近期原始消息并注入摘要。 |
| FR-MEM-06 | 功能 | 支持 MemoryEvent 与 TaskTrace 记录会话、Planner 动作、工具调用、画布产物和用户反馈，作为记忆分析与技能迭代的数据底座。 |
| FR-SKILL-01 | 功能 | 支持技能库 Tab，包含我的技能、技能草稿箱和在线技能库；用户可从画布主动保存当前工作流为技能草稿。 |
| FR-SKILL-02 | 功能 | 支持启用技能草稿：生成用户级 `.agent/skills/user-generated/*/SKILL.md`、`skill.json`，并登记到 `installed_skills`。 |
| FR-SKILL-03 | 功能 | 支持已启用技能召回：对话显式选择技能或自然语言命中技能索引时，Planner 加载对应 SKILL.md/manifest 并注入执行协议。 |
| FR-SKILL-04 | 功能 | 支持技能执行 trace 与迭代：记录 SkillRun、反馈事件、失败/纠正信号，生成 SkillPatchProposal，用户确认后更新 SKILL.md、skill.json、CHANGELOG 和版本号。 |
...
- **小红书笔记搜索**：
  - 用户可通过对话输入关键词搜索小红书笔记。
  - 搜索关键词由用户输入中的显式搜索词和任务目标生成，保留用户指定的主题、时间、地点和风格描述。
  - 调用 MCP 工具 `search_feeds`，返回笔记列表（标题、链接、作者、互动数据）。
  - 搜索列表返回后，前 `limit` 条基础笔记信息会立即写入灵感库，并通过 SSE 事件推送前端刷新。
  - 搜索结果保留标题、封面、互动数据、xsec_token 和来源 ID，供后续分析和详情增强使用。

- **笔记详情增强**：
  - 详情增强通过环境变量 `XHS_ENABLE_DETAIL_ENRICH=1` 启用。
  - 启用后通过 `get_feed_detail` 获取正文、图片列表、标签、发布时间、地点等元数据。
  - 详情页验证、页面无数据、网络超时等情况会记录错误类型，基础笔记入库链路保持可用。
...
- **MCP 工具层**：
  - `search_feeds(keyword)` - 搜索笔记并返回基础列表数据
  - `get_feed_detail(feed_id, xsec_token)` - 可选详情增强
  - `publish_note(title, content, image_urls)` - 发布笔记
  - `get_self_info()` - 获取当前用户信息

### 4.12 Skill 系统与电商生图 Skill（FR-16 / FR-SKILL-01 ~ FR-SKILL-04）

- **Skill 系统架构**：
  - Skill 是可插拔的业务能力模块，位于 `.agent/skills/` 目录。
  - 每个 Skill 至少包含：`SKILL.md`（执行协议文档）与 `skill.json`（manifest，用于前端展示、召回规则和版本管理）。
  - 系统内置 Skill 与用户生成 Skill 采用统一加载入口：`activeSkill` / enabled skill index → 读取 manifest → 加载 SKILL.md → 注入 Planner prompt。
  - 用户生成 Skill 安装到 `.agent/skills/user-generated/{name}-{candidate_id}/`，并写入 `installed_skills` 表。

- **技能库前端能力**：
  - 顶部 Tab 包含「技能库」，内部 Tab 顺序为「我的技能 / 技能草稿箱 / 在线技能库」。
  - 技能补丁建议在对话消息中以确认卡片呈现，也可通过补丁建议接口查询。
  - 画布「增加项目」旁提供「保存为技能草稿」入口，可将当前画布节点、连线、项目名和会话 trace 生成 SkillCandidate。
  - 技能草稿可编辑名称、触发示例、工作流步骤、依赖工具和草稿 SKILL.md。
  - 点击「启用草稿」后生成真实 SKILL.md/skill.json，并在「已启用技能」中展示。

- **技能召回入口**：
  - 入口 A：用户主动在技能库点击「使用技能」，前端写入 `activeSkill`，后端直接加载对应 SKILL.md。
  - 入口 B：对话面板中的 Skill 推荐标签，按召回置信度展示；高置信度显示主要按钮，中置信度显示「可尝试」，低置信度不打扰用户。
  - 入口 C：自然语言自动召回，Planner 注入轻量 enabled skill index，命中后再加载完整 SKILL.md。
  - 入口 D：特殊上下文召回，如上传商品图后提示电商生图 Skill。

- **通用召回规则**：
  - 每个 manifest 可定义 `recall_keywords`、`recall_rules`、`required_slots`、`auto_run_policy`。
  - 关键词不是简单任意命中，而是按分组判断：`must_have_any`、`topic_any`、`action_any`、`output_any`、`negative_any`。
  - 高置信度：满足核心主题 + 动作 + 输出，且无 negative；可以自动进入技能或显示主按钮。
  - 中置信度：只满足部分主题/动作，显示「可尝试」。
  - 低置信度：只命中泛词或命中 negative，不展示。
  - 缺必填参数时允许进入技能，但必须先追问，不直接执行高风险工具。
...
- **Skill 探测机制**：
  - 商品图上传或用户输入“电商主图/商品图/产品图/生成电商图片”等明确意图时触发。
  - 动态构建增强 Prompt，注入分类配置、参考图库和执行边界。
  - 支持数字选择 Fast Path（用户回复纯数字自动映射到对应模版）。

- **小红书活动合集用户生成 Skill 示例**：
  - 触发输入示例：`搜索小红书上海五月热门市集，总结9个，粉色风格模版`。
  - 必须提供：城市/区域、时间范围、活动主题、活动数量。
  - 可选提供：每页几个活动、免费优先/适合拍照/亲子友好、是否优先使用已勾选灵感笔记。
  - 执行边界：先搜索小红书或复用已选灵感库笔记；搜索列表基础信息即时入库并推送到灵感库；随后生成可溯源活动合集草稿；用户确认生图并选择模板后，才创建内容输入、模板选择、精细编排节点。
  - 模板选择后的内容输入节点优先使用活动合集草稿或结构化结果，完整草稿可作为结构化超时兜底内容。

- **技能迭代机制**：
  - 每次技能执行写入 `skill_runs`，记录输入、上下文、Planner actions、工具调用、画布变化、最终输出和状态。
  - 用户纠正、失败重试、生成结果不符合预期等事件写入 `skill_feedback_events`。
  - 后端生成 `skill_patch_proposals`，包含问题摘要、补丁类型、建议改动、风险等级和来源运行。
  - 用户确认后更新 SKILL.md 的 `Learned Updates`、skill.json 的 `version/learned_updates`、CHANGELOG.md，并提升 installed skill 版本。
...
### 4.23 用户记忆系统（FR-MEM-01 ~ FR-MEM-06）

#### 4.23.1 记忆分层架构

采用**手动录入 + 自动注入 + 自动压缩 + 行为事件沉淀**机制，分为四层：
...
| **结构化长期记忆** | UserMemory 表 (preference/rejection/workflow/...) | JSON 键值对 | 按需 | 后端保留，按需开放 |
| **短期会话记忆** | ConversationSummary + MemoryEvent | 摘要 + 原始事件 | 当前会话内 | 自动触发 |
...
对话请求
  └─► dialogue_routes.py
        ...
        └─► run_planner(memory_summary=文本块)
              └─► router.py 将 memory_summary 前置拼接到 ROUTER_PROMPT
```

#### 4.23.5 短期会话压缩机制（FR-MEM-05）

- 目标：避免长会话上下文膨胀，同时保留任务状态、用户决策、工具结果、待办和当前 Skill/项目上下文。
- 触发条件满足任一即可进入压缩判断：
  - token 估算达到软阈值（默认约 10k）并且出现任务阶段完成、工具链完成、项目切换、技能切换等弱触发。
  - token 估算达到硬阈值（默认约 14k）时强制压缩。
  - 消息数达到兜底阈值（默认 80），用于 token 估算失准时兜底。
- 保留策略：
  - 正常压缩保留最近 8 条原始消息。
  - 硬阈值压缩保留最近 4 条原始消息。
  - 被压缩区间写入 `conversation_summaries`，`structured_summary` 记录 trigger、estimated_tokens、preserved_recent_count、last_action 等元信息。
- 压缩内容要求：
  - 保留用户目标、已完成步骤、失败点、用户偏好、选中的素材/模板、画布节点引用、下一步待办。
  - 不把已过期的工具噪声完整塞回 prompt。

#### 4.23.6 行为事件与任务 trace（FR-MEM-06）

- `MemoryEvent` 记录会话级事件：用户消息、Planner action、工具调用、工具结果、画布动作、产物创建、用户反馈。
- `TaskTrace` 聚合一次任务运行：用户目标、Planner actions、产物列表、结果状态、失败原因。
- 这些数据同时服务于三类能力：
  - 记忆回流：从长期行为中提取偏好和工作流模式。
  - 技能草稿：把用户主动保存的画布和最近 trace 转成 SkillCandidate。
  - 技能迭代：把失败和纠正转换成可确认的 SkillPatchProposal。
...
**前端交互**：
- Header Tab 顺序为「画布 / 知识库 / 灵感库 / 生图库 / 技能库 / 我的项目」
- 项目名称不显示在 Logo 旁，项目管理集中在「我的项目」Tab 和画布状态中
...
**触发方式**：
- 前端可通过 `POST /api/v1/projects/analyze-memory` 主动触发记忆分析。
- 分析过程不阻塞用户操作；后端使用分析游标避免重复处理已分析事件。
...
- LlamaIndex, ChromaDB, rank-bm25, llama-index-vector-stores-chroma, llama-index-retrievers-bm25
...
| FR-MEM-05 | `backend/app/agents/planner/summarizer.py` + `backend/app/memory/models.py` | token 阈值、阶段完成、工具链完成、项目/技能切换触发压缩 |
| FR-MEM-06 | `backend/app/memory/models.py` + `backend/app/api/dialogue_routes.py` | MemoryEvent / TaskTrace 写入与追溯 |
...
| FR-SKILL-01 | `frontend/src/components/skills/SkillLibraryPanel.js` + `backend/app/api/skill_routes.py` | 技能库、草稿箱、保存当前画布为技能草稿 |
| FR-SKILL-02 | `backend/app/api/skill_routes.py` + `.agent/skills/user-generated/` | 启用草稿生成 SKILL.md / skill.json 并注册 installed skill |
| FR-SKILL-03 | `backend/app/agents/planner/skills.py` + `backend/app/agents/planner/router.py` | activeSkill 与自然语言自动召回验证 |
| FR-SKILL-04 | `backend/app/api/skill_routes.py` + `backend/app/memory/models.py` | SkillRun、反馈事件、PatchProposal、确认应用与版本更新 |
