...
| LlamaIndex Chroma/BM25 插件 | 随 `requirements.txt` 安装 | RAG 混合检索依赖 `llama-index-vector-stores-chroma` 与 `llama-index-retrievers-bm25`；BM25 插件缺失时可降级到 `rank-bm25` |
...
# 如果 RAG 检索提示缺少 LlamaIndex 插件，可单独补装
pip install llama-index-vector-stores-chroma llama-index-retrievers-bm25
